...
const playerRepository = __importStar(require("../repositories/playerRepository"));
...
const VALID_POSITIONS = Object.values(enums_1.PlayerPosition);
...
async function listPlayers(teamId, userId, filters) {
    await (0, teamAuthorization_1.assertTeamOwnership)(teamId, userId);
    const resolvedPosition = filters?.position ? normalizePosition(filters.position) : undefined;
    return playerRepository.findPlayersByTeam(teamId, {
        search: filters?.search,
        position: resolvedPosition,
    });
}
async function createPlayer(teamId, userId, input) {
    await (0, teamAuthorization_1.assertTeamOwnership)(teamId, userId);
    const firstName = input.firstName?.trim();
    const lastName = input.lastName?.trim();
    if (!firstName)
        throw err(400, 'firstName is required');
    if (!lastName)
        throw err(400, 'lastName is required');
    if (!Number.isInteger(input.jerseyNumber) || input.jerseyNumber < 0) {
        throw err(400, 'jerseyNumber must be a non-negative integer');
    }
    const existing = await playerRepository.findByTeamAndJersey(teamId, input.jerseyNumber);
    if (existing)
...
async function updatePlayer(playerId, userId, input) {
    ...
    if (input.jerseyNumber !== undefined &&
        ...
        const clash = await playerRepository.findByTeamAndJersey(player.teamId, input.jerseyNumber);
    ...
    if (input.firstName !== undefined) {
        ...
        if (!v)
            throw err(400, 'firstName cannot be empty');
        patch.firstName = v;
    }
    if (input.lastName !== undefined) {
        const v = input.lastName.trim();
        if (!v)
            throw err(400, 'lastName cannot be empty');
        patch.lastName = v;
    }
...
async function deletePlayer(playerId, userId) {
    await (0, teamAuthorization_1.assertPlayerOwnership)(playerId, userId);
...
async function getCareerStats(playerId, userId) {
    const player = await (0, teamAuthorization_1.assertPlayerOwnership)(playerId, userId);
    const stats = await prisma_1.prisma.frisbeePlayerStat.findUnique({
        where: { playerId },
    });
    const zero = {
    ...
    const teamAverages = await prisma_1.prisma.frisbeePlayerStat.aggregate({
...
// Pass network
// ---------------------------------------------------------------------------
async function getPassNetwork(playerId, userId) {
    await (0, teamAuthorization_1.assertPlayerOwnership)(playerId, userId);
    const edges = await prisma_1.prisma.playerPassEdge.findMany({
